if(!localStorage.getItem('following-start')){
    localStorage.setItem('following-start',0)
}
...
function load() {
    ...
    const start = localStorage.getItem('following-start');
    const end = start*1 + quantity - 1;
    starting = end + 1;

    document.getElementById("posts").innerHTML='';
    // Get new posts and add posts
    fetch(`/following_posts?start=${start}&end=${end}`)
...
function next_post(){
    start = localStorage.getItem('following-start');
    localStorage.setItem('following-start',start*1+10)
    localStorage.setItem('following-start',start*1+10);
    load();
}
