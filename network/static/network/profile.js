if(!localStorage.getItem('prof-starting')){
  localStorage.setItem('prof-starting',0)
}
...
function load() {
  ...
  const start = localStorage.getItem('prof-starting');;
  const end = start + quantity - 1;
  starting = end + 1;

  document.getElementById("Prof-posts").innerHTML='';
  // Get new posts and add posts
  fetch(`/prof_posts?start=${start}&end=${end}`)
...
function show_post(post) {
  ...
  document.getElementById("Prof-posts").appendChild(box);
  item1.append(title);
...
function next_post(){
  start = localStorage.getItem('prof-starting');
  localStorage.setItem('prof-starting',start*1+10)
  localStorage.setItem('prof-starting',start*1+10);
  load();
}
